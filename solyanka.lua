...
script_version("08.07.2023")
script_version_number(21)
local macros =
    "https://script.google.com/macros/s/AKfycbyO5cG_ROl_Ar2T2_q6FkYNFdCEKo82Jsr41tzBA5cD7uD05ka46GwxZ3oG1VnXSas/exec?do"
local req_index = 0
...
local config = {
    ...
    hotkey = {psiho = '0', fast = '0'},
    values = {password = "", tag = "|| C.О.П.Т. ||", clist = 12}
}
...
local zv =
    { -- массив наименований званий согласно их реальному порядку
...
local zvskl = { -- массив званий в склонении
    "рядового", "eфрейтора",
    "младшего сержанта", "сержанта",
    "старшего сержанта", "старшины",
    "прапорщика", "младшего лейтенанта",
    "лейтенанта", "старшего лейтенанта",
    "капитана", "майора", "подполковника",
    "полковника", "генерала"
}

local posts =
    { -- координаты контрольно-пропускных пунктов Army LV
...
function main()
    ...
    for n, fn in pairs(servers) do
        if serverName:find(n) then
            server = fn
            break
        end
    end
    ...
    mynick = sampGetPlayerNickname(select(2, sampGetPlayerIdByCharHandle(
                                              PLAYER_PED)))
    ...
    AdressFolder = string.format("%s\\config\\solyanka\\%s\\%s",
                                 thisScript().directory, server, mynick)
    ...
    belka_folder = string.format("%s\\config\\config.ini",
    ...
    script.password = solyanka_ini.values.password ~= "" and
                          solyanka_ini.values.password .. " " or nil -- пароль из локального конфига скрипта
    ...
    sampRegisterChatCommand('sologin', function(p)
        lua_thread.create(function()
            if p == nil or p == "" then
                if not showdialog(3,
                                  u8:decode "Введите ваш пароль:",
                                  u8:decode "Будьте осторожны при вводе!",
                                  u8:decode "Отправить") then
                    script.sendMessage(
                        "Ошибка при создании диалогового окна.")
                    return
                end
                local res = waitForChooseInDialog(3)
                if not res or res == "" then
                    script.sendMessage(
                        "Диалог был закрыт. Попробуйте /sologin [пароль]")
                    return
                end
                p = res
            end
            script.sendMessage(
                "Осуществляю попытку авторизации...")
            script.logging(p)
        end)
    end)
    ...
    togglebools = {
        sex = solyanka_ini.bools.sex and imgui.ImBool(true) or
            imgui.ImBool(false),
        cl7 = solyanka_ini.bools.cl7 and imgui.ImBool(true) or
            imgui.ImBool(false),
        autocl = solyanka_ini.bools.autocl and imgui.ImBool(true) or
            imgui.ImBool(false),
        seedo = solyanka_ini.bools.seedo and imgui.ImBool(true) or
            imgui.ImBool(false),
        psihosbiv = solyanka_ini.bools.psihosbiv and imgui.ImBool(true) or
            imgui.ImBool(false),
        house = solyanka_ini.bools.house and imgui.ImBool(true) or
            imgui.ImBool(false)
    }
    ...
    script.loaded = true
    script.sendMessage("Скрипт by " .. unpack(thisScript().authors) ..
                           " запущен. Открыть главное меню - /solyanka")
    ...
    while true do
        ...
        if suspendkeys == 2 then
            rkeys.registerHotKey(makeHotKey('psiho'), true, function()
                ...
                if var.satiety > 20 then
                    ...
                    chatManager.addMessageToQueue(
                        (solyanka_ini.bools.sbivgrib and "/grib" or "/fish") ..
                            " eat")
        ...
        if isKeyDown(makeHotKey('fast')[1]) and not menu.main.v and
            not sampIsChatInputActive() and not sampIsDialogActive(-1) and
            ...
            if not menu.interaction.v then
                if menu.target.v then
                    if targetNick ~= nil and targetId ~= nil then
                        ...
                        local found = false
                        downloadUrlToFile(
                            "http://srp-addons.ru/om/fraction/Army%20LV", temp,
                            function(_, status)
                                if (status == 58) then
                                    local file = io.open(temp, "r")
                                    playerNick =
                                        sampGetPlayerNickname(select(2,
                                                                     sampGetPlayerIdByCharHandle(
                                                                         PLAYER_PED)))
                                    for line in file:lines() do
                                        line = encoding.UTF8:decode(line)
                                        local offrank, offtm, offwm, offdate =
                                            line:match('%["' .. offNick ..
                                                           '",(%d+),%[(%d+),(%d+)%],"(%d+/%d+/%d+ %d+%:%d+%:%d+)"%]')
                                        local myrank, mytm, mywm, mydate =
                                            line:match('%["' .. playerNick ..
                                                           '",(%d+),%[(%d+),(%d+)%],"(%d+/%d+/%d+ %d+%:%d+%:%d+)"%]')
                                        if tonumber(myrank) ~= nil and
                                            tonumber(offrank) ~= nil then
                                            found = true
                                    ...
                                    if not found then
                                        script.sendMessage(
                                            "Произошла ошибка. Не удалось получить ранг цели или ваш ранг!")
                                    end
                                    file:close()
                                    os.remove(temp)
                                    if found then
                                    ...
                                    if (os.time() - time > 10) then
                                        script.sendMessage(
                                            "Превышено время загрузки файла, повторите попытку",
                                            0xFFFFFFFF)
                                        return
                                    end
                                end
        ...
        for nick, t in pairs(alreadyUd) do
            if (os.time() - t) >= 30 then
                alreadyUd[nick] = nil
            else
                if checkpoint[nick] ~= nil then
                    local surname = nick:match(".*%_(.*)")
                    f(
                        "В часть прибыл " .. checkpoint[nick] .. " " ..
                            surname)
                    chatManager.addMessageToQueue(
                        "Товарищ " .. checkpoint[nick] .. " " .. surname ..
                            ", можете двигаться далее!")
                    alreadyUd[nick] = nil
                    checkpoint[nick] = nil
                end
            end
...
function imgui.OnDrawFrame()
    if menu.main.v then -- меню скрипта
        ...
        imgui.Begin(thisScript().name .. ' v' .. script.v.num .. ' от ' ..
                        script.v.date, menu.main,
        ...
        if imgui.Button("Настройки",
                        imgui.ImVec2(mainButtonsWidth, 35.0)) then
            menu.settings.v = true
            menu.information.v = false
        ...
        if imgui.Button("Информация",
                        imgui.ImVec2(mainButtonsWidth, 35.0)) then
            menu.settings.v = false
            menu.information.v = true
            menu.commands.v = false
            menu.admin.v = false
        end
        if script.admin.status then
            imgui.SameLine()
            if imgui.Button("Панель администратора",
                            imgui.ImVec2(mainButtonsWidth, 35.0)) then
                menu.settings.v = false
                menu.information.v = false
                menu.commands.v = false
        ...
        if menu.settings.v and not menu.information.v then
            ...
            if imgui.InputText('##tag', buffer.tag) then
                ...
                tag =
                    solyanka_ini.values.tag ~= "" and solyanka_ini.values.tag ..
                        " " or ""
                inicfg.save(solyanka_ini, settings)
            end
            imgui.PopItemWidth()
            ...
            imgui.Text("Ваше звание из srp-addons: " ..
                           (script.zv ~= nil and script.zv or
                               "Неизвестно"))
            imgui.TextColoredRGB("Количество отметок: " ..
                                     (script.otm ~= nil and "{B30000}" ..
                                         script.otm or "Неизвестно"))
            imgui.EndChild()
            ...
            imgui.SameLine()
            imgui.Text(
                "Автоматически вводить /clist 7 на базе после захода в игру")
            ...
            imgui.SameLine()
            imgui.Text(
                "'Автоклист' - автоматически вводить клист после: нач.раб.дня, завершения раб.дня, смерти, разгрузки на складе с кодом 0")
            ...
            imgui.SameLine()
            imgui.Text(
                "Отправлять /seedo при входящих SMS [Если SMS от военнослужащего - напишет его звание]")
            ...
            imgui.SameLine()
            imgui.Text(
                "Сбивать анимацию приёма психохила чатом" ..
                    (solyanka_ini.bools.psihosbiv and
                        " | Если мало HP, то пополнять сытость:" or
                        ""))
            if solyanka_ini.bools.psihosbiv then
                imgui.SameLine()
                imgui.PushItemWidth(200)
                if imgui.Combo("##Combo1", buffer.sbiv, sbiv) then
                    solyanka_ini.bools.sbivgrib =
                        buffer.sbiv.v == 1 and true or false
                    inicfg.save(solyanka_ini, settings)
                end
                imgui.PopItemWidth()
            end
            ...
            imgui.SameLine()
            imgui.Text(
                "Сохранять дату слёта недвижимости в базу данных скрипта и упоминать о её слёте")
        ...
        if not menu.settings.v and menu.information.v and not menu.admin.v then
            imgui.Text(
                "Данный скрипт является кучей всякого говна")
            imgui.Text("Автор: Cody_Webb | Telegram: @ibm287")
            imgui.SameLine()
            if imgui.Button("Написать разработчику",
                            imgui.ImVec2(180.0, 23.0)) then
                os.execute('explorer "https://t.me/ibm287"')
            end
            imgui.NewLine()
            imgui.Text(
                "Все настройки автоматически сохраняются в файл:\nmoonloader//config//solyanka//Server//Nick_Name")
            imgui.NewLine()
            imgui.Text(
        ...
        if not menu.settings.v and not menu.information.v and menu.admin.v then
            ...
            if script.admin.info ~= nil then
                ...
                for k, v in ipairs(script.admin.info.access) do
                    ...
                    if imgui.Selectable(v.nick) then
                ...
                imgui.Text(
                    "Происходит загрузка информации, ожидайте...")
            end
            imgui.EndChild()
            if imgui.Button(
                "Обновить информацию из базы данных",
                imgui.ImVec2(270.0, 26.0)) then
                lua_thread.create(function()
                    script.updateAdminInformation()
                end)
            end
            if imgui.Button(
                "Добавить пользователя скрипта",
                imgui.ImVec2(270.0, 26.0)) then menu.add.v = true end
        end
        ...
        if menu.add.v then
            ...
            imgui.SetNextWindowSize(imgui.ImVec2(600, 115),
                                    imgui.Cond.FirstUseEver)
            imgui.Begin("Ввод ID/Nickname", menu.add,
                        imgui.WindowFlags.NoCollapse +
                            imgui.WindowFlags.NoResize +
                            imgui.WindowFlags.NoScrollbar)
            imgui.Text(
                "Введите в текстовую строку ID или ник игрока, которого вы хотите добавить в базу данных")
                ...
                lua_thread.create(function()
                    ...
                    if tonumber(res) ~= nil then
                        ...
                        if not sampIsPlayerConnected(id) then
                            script.sendMessage("Игрок не найден!")
                        end
                    ...
                    response = req(
                                   macros .. "=add&nick=" .. nick .. "&admin=" ..
                                       mynick .. "&password=" .. script.password)
                    if response == "Access denied" then
                        script.sendMessage(
                            mynick:gsub("_", " ") ..
                                " — доступ к скрипту отсутствует!")
                        script.noaccess = true
                        thisScript():unload()
                        return
                    end
                    if response == "Wrong password" then
                        script.sendMessage("Неверный пароль!")
                    end
                    if response == "You're not admin" then
                        script.sendMessage(
                            "У вас отсутвуют права администратора скрипта!")
                    end
                    if response == "Successfuly added" then
                        script.sendMessage(
                            "Игрок был добавлен в базу данных скрипта!")
                        script.updateAdminInformation()
                    end
                end)
            end
        ...
        if menu.remove.v then
            imgui.SetNextWindowPos(imgui.ImVec2(sw / 2, sh / 2),
                                   imgui.Cond.FirstUseEver,
                                   imgui.ImVec2(0.5, 0.5))
            imgui.SetNextWindowSize(imgui.ImVec2(600, 75),
                                    imgui.Cond.FirstUseEver)
            imgui.Begin("Удаление игрока " .. menu.removenick ..
                            " из базы данных скрипта",
                        menu.remove,
                        imgui.WindowFlags.NoCollapse +
                            imgui.WindowFlags.NoResize +
                            imgui.WindowFlags.NoScrollbar)
            imgui.Text(
                "Нажмите 'Согласен' если вы действительно хотите удалить пользователя скрипта")
            imgui.PushItemWidth(300)
            if imgui.Button("Согласен", imgui.ImVec2(100.0, 23.0)) then
                lua_thread.create(function()
                    ...
                    script.sendMessage("Выполняю...")
                    response = req(
                                   macros .. "=remove&nick=" .. menu.removenick ..
                                       "&admin=" .. mynick .. "&password=" ..
                                       script.password)
                    if response == "Access denied" then
                        script.sendMessage(
                            mynick:gsub("_", " ") ..
                    ...
                    if response == "Successfuly removed" then
                        script.sendMessage(
                            "Игрок был удалён из базы данных скрипта!")
                        script.updateAdminInformation()
                    end
                    if response == "Unsuccessfuly removed" then
                        script.sendMessage(
                            "Не удалось удалить игрока из базы данных!")
                    end
                end)
            end
            imgui.PopItemWidth()
        ...
        if menu.change.v then
            ...
            imgui.Begin(
                "Изменить права пользователя " ..
                    menu.changenick ..
                    " в базе данных скрипта", menu.change,
                imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize +
                    imgui.WindowFlags.NoScrollbar)
            imgui.Text(
                "Нажмите 'Согласен' если вы действительно хотите изменить права")
            imgui.PushItemWidth(300)
            if imgui.Button("Согласен", imgui.ImVec2(100.0, 23.0)) then
                lua_thread.create(function()
                    menu.change.v = false
                    script.sendMessage("Выполняю...")
                    response = req(
                                   macros .. "=change&nick=" .. menu.changenick ..
                                       "&admin=" .. mynick .. "&password=" ..
                                       script.password)
                    if response == "Access denied" then
                        script.sendMessage(
                    ...
                    local newstatus = response:match(
                                          "Successfuly changed as (.*)")
                    if newstatus ~= nil then
                        script.sendMessage(
                            newstatus == "admin" and
                                "Пользователь был успешно назначен администратором скрипта" or
                                "Пользователь был лишён прав администратора")
                        script.updateAdminInformation()
                    else
                        script.sendMessage(
        ...
        if menu.commands.v then
            imgui.SetNextWindowPos(imgui.ImVec2(sw / 2, sh / 2),
                                   imgui.Cond.FirstUseEver,
                                   imgui.ImVec2(0.5, 0.5))
            imgui.SetNextWindowSize(imgui.ImVec2(900, 300),
                                    imgui.Cond.FirstUseEver,
                                    imgui.WindowFlags.NoCollapse +
                                        imgui.WindowFlags.NoResize +
                                        imgui.WindowFlags.NoScrollbar)
            imgui.Begin("Все команды скрипта", menu.commands,
                        imgui.WindowFlags.NoCollapse +
                            imgui.WindowFlags.NoResize +
                            imgui.WindowFlags.NoScrollbar)
            imgui.Text(
        ...
        if not menu.admin.v then
            ...
            for i = 0, 1000 do
                if sampIsPlayerConnected(i) and sampGetPlayerScore(i) ~= 0 then
                    if sampGetPlayerNickname(i) == "Cody_Webb" then
                        ...
                        if imgui.Button(ownernick .. "[" .. i ..
                                            "] сейчас в сети",
                                        imgui.ImVec2(260.0, 30.0)) then
                            chatManager.addMessageToQueue("/sms " .. i ..
                                                              " Я пользуюсь solyanka.lua, большое спасибо")
                        end
                        found = true
                    end
                end
            ...
            if not found then
                if imgui.Button("Cody Webb сейчас не в сети",
                                imgui.ImVec2(260.0, 30.0)) then
                    script.sendMessage(
                        "Cody Webb играет на Revolution (сейчас не онлайн)")
                end
            end

            if imgui.Button("Все команды скрипта",
                            imgui.ImVec2(170.0, 23.0)) then
                menu.commands.v = true
            end

            imgui.SameLine(635)

            if imgui.Button("Перезагрузить скрипт",
                            imgui.ImVec2(170.0, 23.0)) then
                showCursor(false)
                script.reload = true
                thisScript():reload()
    ...
    if menu.fast.v then -- быстрое меню скрипта
        ...
        imgui.SetNextWindowPos(imgui.ImVec2(sw / 2, sh / 2),
                               imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
        imgui.SetNextWindowSize(imgui.ImVec2(1000, 250),
                                imgui.Cond.FirstUseEver,
                                imgui.WindowFlags.NoCollapse +
                                    imgui.WindowFlags.NoResize +
                                    imgui.WindowFlags.NoScrollbar)
        imgui.Begin("Быстрое меню скрипта", menu.fast,
                    imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize +
                        imgui.WindowFlags.NoScrollbar)
        ...
        local strings = {
            [1] = {
                ...
                strings = {
                    "/me включил" .. a ..
                        "камеру на бронежилете",
                    "/do Камера включена и записывает всё происходящее на удалённый сервер."
                }
            },
            [2] = {
                name = "Статус записи",
            ...
            [3] = {
                ...
                strings = {
                    "/me выключил" .. a .. "камеру",
                    "/do Камера выключена, запись сохранена на удалённый сервер."
                }
            },
            [4] = {
                name = "Стянуть маску",
                strings = {
                    "/me резко стянул" .. a ..
                        "маску с лица человека",
                    "/do Лицо полностью видно."
                }
            },
            [5] = {
                name = "Запросить полицейских в департамент",
                strings = {
                    "/dep SAPD, требуется экипаж в секторе " ..
                        sector()
                }
            },
            [6] = {
                name = "Поздароваться с военнослужащими",
                strings = {
                    "/f " .. tag ..
    ...
    if menu.interaction.v then -- меню взаимодействия с военнослужащим
        ...
        imgui.SetNextWindowPos(imgui.ImVec2(sw / 2, sh / 2),
                               imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
        local prank, psurname, myrank, mysurname = zv[targetRank],
                                                   targetNick:match(".*%_(.*)"),
                                                   zv[playerRank],
                                                   playerNick:match(".*%_(.*)")
        imgui.SetNextWindowSize(imgui.ImVec2(1000, 250),
                                ...
                                imgui.WindowFlags.NoCollapse +
                                    ...
                                    imgui.WindowFlags.NoScrollbar)
        imgui.Begin(
            "Меню взаимодействия с военнослужащим: " ..
                prank .. " " .. targetNick:gsub("_", " "), menu.interaction,
            imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize +
                imgui.WindowFlags.NoScrollbar)
        imgui.BeginChild('interaction', imgui.ImVec2(985, 195), true)
        local strings = {
            [1] = "Здравия желаю, товарищ " .. prank .. " " ..
                psurname .. "!",
            [2] = "Товарищ " .. prank .. " " .. psurname,
            [3] = "Товарищ " .. prank .. " " .. psurname ..
                " разрешите обратится? Товарищ " ..
                myrank .. " " .. mysurname,
            [4] = "Товарищ " .. prank .. " " .. psurname ..
                " разрешите встать в строй?",
            [5] = "Товарищ " .. prank .. " " .. psurname ..
                ", товарищ " .. myrank .. " " .. mysurname ..
    ...
    if menu.target.v then
        ...
        if targetId ~= nil and targetNick ~= nil then
            local targetClist = "{" ..
                                    ("%06x"):format(
                                        bit.band(sampGetPlayerColor(targetId),
                                                 0xFFFFFF)) .. "}"
            imgui.TextColoredRGB('Текущая цель: ' ..
                                     (targetClist ~= nil and targetClist or '') ..
                                     targetNick .. '[' .. targetId .. ']')
            imgui.TextColoredRGB('O' ..
                                     ' - меню взаимодействия')
        end
        imgui.End()
    end
...
function ev.onServerMessage(col, text)
    if script.loaded then
        if waitforchangeclist and col == -1 and
            text:match(u8:decode "Цвет выбран") ~= nil then
            waitforchangeclist = false
        end
        if col == -356056833 and text:match(
            u8:decode "^ Для восстановления доступа нажмите клавишу %'F6%' и введите %'%/restoreAccess%'") then
            var.needtocl = true
        ...
        if solyanka_ini.bools.autocl and col == 1790050303 then
            if text:match(u8:decode "^ Рабочий день окончен") then
                chatManager.addMessageToQueue("/clist 7")
                chatManager.addMessageToQueue(
                    "/me надел" .. a .. u8(belka_ini.UserClist[7]))
            elseif text:match(u8:decode "^ Рабочий день начат") then
                ...
                if cl ~= nil then
                    ...
                    chatManager.addMessageToQueue(
                        "/me надел" .. a .. u8(belka_ini.UserClist[cl]))
                end
            end
        end
        if solyanka_ini.bools.seedo and col == -65281 then
            local smstxt, smsnick, smsid = text:match(
            ...
            if smstxt ~= nil and smsnick ~= nil and smsid ~= nil then
                ...
                if checkid == tonumber(smsid) and sampIsPlayerConnected(checkid) then
                    chatManager.addMessageToQueue(
                        "/seedo Поступило сообщение " ..
                            (script.offmembers[smsnick] ~= nil and "от " ..
                                zvskl[script.offmembers[smsnick].rank] or
                                "на телефон") .. "")
                end
            end
        end
        if col == -1342193921 then
            if text:match(
                u8:decode "^ Сытость полностью восстановлена%. У вас осталось %d+ %/ %d+ пачек рыбы$") then
                var.satiety = nil
            end
            local satiety = tonumber(text:match(
                                         u8:decode "^ Сытость пополнена до (%d+)%. У вас осталось %d+%/%d+ готовых грибов$"))
            if satiety ~= nil then var.satiety = satiety end
        end
        if col == -1 then
            local satiety = tonumber(text:match(
                                         u8:decode "^ Здоровье %d+%/%d+%. Сытость (%d+)%/%d+%. У вас осталось %d+%/%d+% психохила$")) or
                                (text:match(
                                    u8:decode "^ Вы истощены%. Здоровье снижено до %d+%/%d+%. У вас осталось %d+%/%d+% психохила$") and
                                    0 or nil)
            if satiety ~= nil then
                var.satiety = satiety
                if solyanka_ini.bools.psihosbiv and
                    not isCharInAnyCar(PLAYER_PED) then
                    lua_thread.create(function()
                        wait(1)
                        sampSendChat(" ")
                    end)
        ...
        local passorg, passrank = text:match(
                                      u8:decode " Организация%: (.*)%s+Должность%: (.*)")
        if passorg ~= nil and passrank ~= nil and col == -169954305 then
            ...
            for i, v in pairs(posts) do
                if (x >= v.x1) and (y >= v.y1) and (x <= v.x2) and (y <= v.y2) then
                    if not passrank:match("Майор") and
                        not passrank:match("[Пп]олковник") and
                        not passrank:match("Генерал") then
                        local ranksnesokr = {
                        ...
                        passrank = ranksnesokr[passrank] ~= nil and
                                       ranksnesokr[passrank] or passrank
                        lua_thread.create(function()
                            local A_Index = 0
                            while true do
                                if A_Index == 5 then
                                        ...
                                        sampGetPlayerNickname(
                                            select(2,
                                                   sampGetPlayerIdByCharHandle(
                                                       PLAYER_PED))) then
                                        local passid =
                                            sampGetPlayerIdByNickname(passnick)
                                        if passid ~= nil then
                                            local result, passskin =
                                                sampGetPlayerSkin(passid)
                                            if result then
                                                if passskin ~= 287 and passskin ~=
                                                    191 then
                                                    if passorg:match("Army LV") then
                                                        local passclist =
                                                            clists.numbers[sampGetPlayerColor(
                                                                passid)]
                                                        if passclist ~= 7 then
                                                            chatManager.addMessageToQueue(
                                                                "Наденьте пожалуйста повязку №7")
                                                            chatManager.addMessageToQueue(
                                                                "/b /clist 7")
                                                        end
                                                        local res, passHandle =
                                                            ...
                                                            if isCharInAnyCar(
                                                                ...
                                                                if getDriverOfCar(
                                                                    ...
                                                                    elseif var ==
                                                                        3 then
                                                                        msg =
                                                                            "А теперь удостоверение"
                                                                    elseif var ==
                                                                        4 then
                                                                        msg =
                                                                            "Отлично, удостоверение"
                                                                    elseif var ==
                                                                        5 then
                                                                        msg =
                                                                            "Теперь нужно удостоверение"
                                                                    elseif var ==
                                                                        6 then
                                                                        msg =
                                                                            "Далее нужно удостоверение"
                                                                    elseif var ==
                                                                        7 then
                                                                        msg =
                                                                            "Для проезда потребуется удостоверение"
                                                                    end
                                                                    chatManager.addMessageToQueue(
                                                                        msg)
                                                                    return
        ...
        local s, sk = text:match(
                          u8:decode "^ На складе (.*)%: (%d+)%/%d+$")
        ...
        if solyanka_ini.bools.house then
            local when = text:match(
                             u8:decode "^ Домашний счёт оплачен до (%d%d%d%d%/%d%d%/%d%d %d%d%:%d%d)")
            if when ~= nil then
                lua_thread.create(function()
                    local datetime = {}
                    datetime.year, datetime.month, datetime.day, datetime.hour =
                        when:match("(%d%d%d%d)%/(%d%d)%/(%d%d) (%d%d%:%d%d)")
                    script.house = datetime -- перевод даты из string в datetime массив
                    script.whenhouse()
                    local response = req(
                                         macros .. "=house&when=" .. when ..
                                             "&nick=" .. mynick .. "&password=" ..
                                             script.password)
                    if response == "Access denied" then
                        script.sendMessage(
                            mynick:gsub("_", " ") ..
                    ...
                    if response == "Wrong password" then
                        script.sendMessage(
                            "Неверный пароль, попробуйте ещё раз — /sologin")
                        return
                    end
                    if response == "House date was changed" then
                        script.sendMessage(
                            "Дата слёта была успешно выгружена в базу данных!")
                        return
                    end
                    if response == "Error" then
                        script.sendMessage(
                            "Не удалось обновить дату слёта в базе данных!")
                        return
                    end
                end)
            end
    ...
    if col == -65281 or col == -10270721 then
        local newnick = text:match(
                            u8:decode 'Ваш новый ник %" (.*) %"%. Укажите его в клиенте SA%-MP%, в поле %"Name%"')
        if newnick ~= nil then
            lua_thread.create(function()
                script.sendMessage(warningprefix ..
                                       "Обнаружена смена игрового ника!")
                script.sendMessage(warningprefix ..
                                       "Скрипт начинает обновление ников в базе данных... {FF0000}НЕ ВЫХОДИТЕ ИЗ ИГРЫ!!!")
                local A_Index = 0
                while true do
                    if A_Index == 20 then break end
                    local text = sampGetChatString(99 - A_Index)

                    local oldnick = text:match(
                                        u8:decode ".* одобрил%(а%) заявку на смену ника%: (.*) %>%> " ..
                                            newnick)
                    if oldnick ~= nil then
                        local response = req(
                                             macros .. "=changenick&newnick=" ..
                                                 newnick .. "&oldnick=" ..
                                                 oldnick .. "&password=" ..
                                                 script.password)
                        if response == "Nick was changed" then
                            script.sendMessage(
                                "Ник в базе данных успешно обновлён!")
                        end
                        if response == "Access denied" then
                            script.sendMessage(
                                mynick:gsub("_", " ") ..
                        ...
                        if response == "Wrong password" then
                            script.sendMessage(
                                "Неверный пароль, свяжитесь с администратором для обновления вручную")
                        end
                        if response == "Error" then
                            script.sendMessage(
                                "Не удалось обновить ник в базе данных, свяжитесь с администратором для обновления вручную")
                        end
                        script.sendMessage(
                            "Можно выходить из игры.")
                        return
                    end
                    A_Index = A_Index + 1
                end
...
function ev.onSendDeathNotification(reason, id)
    if script.loaded then
        if solyanka_ini.bools.autocl then
            local result, sid = sampGetPlayerSkin(select(2,
                                                         sampGetPlayerIdByCharHandle(
                                                             PLAYER_PED)))
...
function sampGetPlayerIdByNickname(name)
    ...
    for i = 0, 1000 do
        if (sampIsPlayerConnected(i) or localId == i) and
            sampGetPlayerNickname(i) == name then return i end
    end
end
...
function f(t)
    if t ~= nil then chatManager.addMessageToQueue("/f " .. tag .. t) end
end
--------------------------------------------------------------------------------------------------------------------------
function cmd_viezd(s) -- доклад о выезде грузовиков снабжения
    lua_thread.create(function()
        if s == "" then
            script.sendMessage(
                "Ошибка! Введите /viezd [число грузовиков] [пункт назначения]")
            return
        end
        local p = {}
        ...
        if tonumber(p[1]) == nil or k[tonumber(p[1])] == nil then
            script.sendMessage(
                "Неверное число грузовиков!")
            return
        end
        ...
        local arr = {
            ...
            ["лс"] = "Police LS",
            ["лспд"] = "Police LS",
            ["lvls"] = "Police LV/Police LS",
            ["lv/ls"] = "Police LV/Police LS",
            ["лвлс"] = "Police LV/Police LS",
            ["лв/лс"] = "Police LV/Police LS",
            ["lslv"] = "Police LV/Police LS",
            ["ls/lv"] = "Police LV/Police LS",
            ["лслв"] = "Police LV/Police LS",
            ["лс/лв"] = "Police LV/Police LS",
            ["sf"] = "г. San-Fierro",
            ["сф"] = "г. San-Fierro",
            ["sfa"] = "Army SF",
            ["сфа"] = "Army SF",
        ...
        if arr[u8(p[2])] == nil then
            script.sendMessage(
                "Неверно указан пункт назначения!")
            return
        end
...
function sampGetPlayerSkin(id)
    if not id or not sampIsPlayerConnected(tonumber(id)) and not tonumber(id) ==
        select(2, sampGetPlayerIdByCharHandle(PLAYER_PED)) then return false end -- проверяем параметр
    local isLocalPlayer = tonumber(id) ==
                              select(2, sampGetPlayerIdByCharHandle(PLAYER_PED)) -- проверяем, является ли цель локальным игроком
    ...
    lua_thread.create(function()
        if p == nil or p == "" then
            if script.password == nil or script.password == "" then
                ...
                if not showdialog(3,
                                  u8:decode "Введите ваш пароль:",
                                  u8:decode "Если авторизируетесь впервые — пароль будет запрошен повторно",
                                  u8:decode "Отправить") then
                    script.sendMessage(
                        "Ошибка при создании диалогового окна.")
                    return
        ...
        print(
            u8:decode "Осуществляю попытку авторизации...")
        local response = req(
                             macros .. "=login&nick=" .. mynick .. "&password=" ..
                                 p)
        ...
        if response ~= nil then
            ...
            if response == "Access denied" then
                script.sendMessage(mynick:gsub("_", " ") ..
                                       " — доступ к скрипту отсутствует!")
                script.noaccess = true
                thisScript():unload()
                return
            ...
            if response == "Need to register" then
                ...
                if not showdialog(1,
                                  u8:decode "Введите ваш новый пароль:",
                                  u8:decode "Будьте осторожны при вводе!",
                                  u8:decode "Отправить") then
                    script.sendMessage(
                        "Ошибка при создании диалогового окна.")
                ...
                p = script.password
                script.sendMessage(
                    "Осуществляю попытку регистрации...")
                local response = req(macros .. "=login&nick=" .. mynick ..
                                         "&password=" .. p .. "&reg=true")
                if response == "Successful registration" then
                    script.sendMessage(
                        "Вы успешно зарегистрировались, сохраняю ваш пароль и перезагружаю скрипт...")
            ...
            print(mynick:gsub("_", " ") ..
                      u8:decode " — успешная авторизация!")
            local info = decodeJson(response)
            if info ~= nil then
                script.admin.status = info.admin.status == "true" and true or
                                          false
                ...
                if info.version > thisScript()['version_num'] then
                    script.sendMessage(updatingprefix ..
                                           "Обнаружена новая версия скрипта от " ..
                                           info.date ..
                ...
                if script.offmembers[mynick] ~= nil then
                    ...
                    if script.zv ~= nil and script.otm ~= nil then
                        script.sendMessage(script.zv .. " " ..
                                               mynick:match(".*%_(.*)") ..
                                               ", у вас {B30000}" ..
                                               script.otm ..
                                               "{FFFAFA} отметок")
                ...
                script.sendMessage(
                    "Произошла ошибка при декодировании json")
                script.unload = true
                thisScript():unload()
            end
        else
...
function script.updateAdminInformation()
    ...
    response = req(macros .. "=get&admin=" .. mynick .. "&password=" ..
                       script.password)
    if response == "Access denied" then
        script.sendMessage(mynick:gsub("_", " ") ..
                               " — доступ к скрипту отсутствует!")
...
function script.whenhouse()
    local days = math.floor((os.difftime(os.time(script.house), os.time())) /
                                3600 / 24)
    if days <= 0 then
        script.sendMessage(warningprefix ..
                               "НЕДВИЖИМОСТЬ ВОЗМОЖНО УЖЕ СЛЕТЕЛА! ЕСЛИ У ВАС НЕТ ДОМА - ОТКЛЮЧИТЕ ФУНКЦИЮ!")
    elseif days <= 7 then
        script.sendMessage(warningprefix ..
                               "Срочно оплатите недвижимость! У вас осталось " ..
                               days .. " дней")
    elseif days > 7 then
        script.sendMessage(
            "Недвижимость слетит через " .. days ..
                " дней")
    end
end
...
function cmd_changepassword(sparams)
    if sparams == "" then
        script.sendMessage(
            "Неправильно блять! Введи — /changepassword [old] [new]")
        return
    end
    local params = {}
    for v in string.gmatch(sparams, "[^%s]+") do table.insert(params, v) end
    if params[1] == nil or params[1] == "" or params[2] == nil or params[2] ==
        "" then
        script.sendMessage(
            "Неправильно блять! Введи — /changepassword [old] [new]")
        return
    ...
    lua_thread.create(function()
        script.sendMessage(
            "Осуществляюю попытку смены пароля...")
        local response = req(macros .. "=changepassword&nick=" .. mynick ..
                                 "&old=" .. params[1] .. "&new=" .. params[2])
        if response == "Access denied" then
            script.sendMessage(mynick:gsub("_", " ") ..
                                   " — доступ к скрипту отсутствует!")
            script.noaccess = true
        ...
        if response == "Wrong old password" then
            script.sendMessage("Старый пароль неверный!")
            return
        end
    end)
end
...
function waitForChooseInDialog(style)
    if style ~= 0 and style ~= 1 and style ~= 2 and style ~= 3 then
        return nil
    end
...
function req(u)
    ...
    while true do
        ...
        os.remove(file_path)
        script.sendMessage(
            "Неудача при выполнении запроса №" ..
                req_index .. ", повторяю попытку...", 0xFF006400)
    end
    return ""
end
...
function download_handler(id, status, p1, p2)
    ...
    if status == dlstatus.STATUS_ENDDOWNLOADDATA then script.complete = true end
end
            ...
            if not script.reload then
                if not script.update then
                    if not script.unload then
                        script.sendMessage(
                            "Скрипт крашнулся: отправьте moonloader.log разработчику")
                    else
                        script.sendMessage(
                            "Скрипт был выгружен")
                    end
                    ...
                    script.sendMessage(updatingprefix ..
                                           "Старый скрипт был выгружен, загружаю обновлённую версию...")
                end
            else
